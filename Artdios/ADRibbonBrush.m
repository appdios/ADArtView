...
@implementation ADRibbonBrush
@synthesize pointsArray;

+ (ADRibbonBrush*)sharedInstance 
{
    static dispatch_once_t once;
    static ADRibbonBrush *sharedInstance;
    dispatch_once(&once, ^{
...
-(void)buildRibbon:(CGPoint)currentPoint{
    ...
    for (int a = 0; a < kPainterLines; a++) {
        SMPainter *painter = [[SMPainter alloc] initWithPoints:currentPoint ap:CGPointZero forBrush:ADBrushTypeRibbon];
...
-(void) pathStart:(CGPoint)point
{
    ...
    glPointSize(3*scale);
    drawCircleTexture(point, 1, 1, 3*scale, FALSE);
...
-(void)pathMoveFromPoint:(CGPoint)beginMain toPoint:(CGPoint)endMain speed:(double)speed
{
    ...
    for (int i=0; i<pcount; i++) {
      ...
      //  CGPoint begin = [painter dp];
        ...
        glColor4f(smcolor.red-i/24, smcolor.green-i/24, smcolor.blue-i/24, 0.05);
